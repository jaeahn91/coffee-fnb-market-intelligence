...
**📄 최신 리포트: [2026년 6월호 (v1 · 트리거 검증호)](reports/monthly_coffee_market_brief_2026_06.md)** — 5월호가 스스로 건 검증 가능한 트리거 3개를 새 데이터로 **판정**하고, 국내 전가 가설을 통계청 CPI로 처음 검증
> 이전호: [2026년 5월호 (v0)](reports/monthly_coffee_market_brief_2026_05.md) — 수입 통계 · 국제 시세 · 환율 · 원산지 리스크 · 국내 F&B 함의, 7개 섹션 (FROZEN)

**반증 가능성은 말이 아니라 실행이다.** 5월호는 "ET 프리미엄 ≥ +$0.8/kg이면 공급 타이트 지속" 같은 임계값을 걸었고, 6월호는 새 데이터로 그것을 정산했다 — A-1 ET 프리미엄 **+$1.03/kg**(지속) · A-2 FX 상쇄 **지속**(국제가 −20%인데 원화 원가 −4.8%) · A-3 물량 **11,411톤**(둔화 강화). 상류 원가 **+114%** vs 외식 커피 CPI **+4.4%** → "생두값 2배 ≠ 판매가 2배" 데이터로 확인.
